...
static void create_GRAPH_global_hashtables(GRAPH_global_context *ggctx)
{
    ...
    vhn = palloc0(vlen + glen + 2);
    ehn = palloc0(elen + glen + 2);
    /* copy in the names */
    strcpy_s(vhn, (vlen + 1), VERTEX_HTAB_NAME);
    strcpy_s(ehn, (elen + 1), EDGE_HTAB_NAME);
    /* add in the graph name */
    vcat_errno = strncat_s(vhn, (vlen + glen + 1), graph_name, glen);
    if (0 != vcat_errno)
    {
        ereport(ERROR,
            (errcode(ERRCODE_DATA_EXCEPTION),
                errmsg("Internal error, attempt to concatenate vertice hash using strncat_s graph name %s reported error number %d",
                    graph_name, vcat_errno)));
    }

    ecat_errno = strncat_s(ehn, (elen + glen + 1), graph_name, glen);
    if (0 != ecat_errno)
    {
        ereport(ERROR,
            (errcode(ERRCODE_DATA_EXCEPTION),
...
PGMODULEEXPORT Datum age_delete_global_graphs(PG_FUNCTION_ARGS)
{
...
PGMODULEEXPORT Datum age_vertex_stats(PG_FUNCTION_ARGS)
{
    GRAPH_global_context *ggctx = NULL;
    vertex_entry *ve = NULL;
...
PGMODULEEXPORT Datum age_graph_stats(PG_FUNCTION_ARGS)
{
    GRAPH_global_context *ggctx = NULL;
    agtype_value *agtv_temp = NULL;
    agtype_value agtv_integer;
