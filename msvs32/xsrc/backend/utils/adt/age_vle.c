...
static void create_VLE_local_state_hashtable(VLE_local_context *vlelctx)
{
    ...
    /* copy in the name */
    strcpy_s(eshn, (elen + 1), EDGE_STATE_HTAB_NAME);
    /* add in the graph name */
    ecat_errno = strncat_s(eshn, (elen + glen + 1), graph_name, glen);
    if (0 != ecat_errno)
    {
        ereport(ERROR,
            (errcode(ERRCODE_DATA_EXCEPTION),
                errmsg("Internal error, attempt to concatenate edge hash using strncat_s graph name %s reported error number %d",
                    graph_name, ecat_errno)));
    }
...
PGMODULEEXPORT Datum age_vle(PG_FUNCTION_ARGS)
{
    FuncCallContext *funcctx;
    VLE_local_context *vlelctx = NULL;
...
PGMODULEEXPORT Datum age_match_two_vle_edges(PG_FUNCTION_ARGS)
{
...
PGMODULEEXPORT Datum age_match_vle_edge_to_id_qual(PG_FUNCTION_ARGS)
{
    int nargs = 0;
    Datum *args = NULL;
    bool *nulls = NULL;
...
PGMODULEEXPORT Datum age_materialize_vle_edges(PG_FUNCTION_ARGS)
{
    agtype *agt_arg_vpc = NULL;
    agtype_value *agtv_array = NULL;
...
PGMODULEEXPORT Datum age_materialize_vle_path(PG_FUNCTION_ARGS)
{
    agtype *agt_arg_vpc = NULL;
...
PGMODULEEXPORT Datum age_match_vle_terminal_edge(PG_FUNCTION_ARGS)
{
    int nargs = 0;
    Datum *args = NULL;
    bool *nulls = NULL;
...
PGMODULEEXPORT Datum age_build_vle_match_edge(PG_FUNCTION_ARGS)
{
...
PGMODULEEXPORT Datum _ag_enforce_edge_uniqueness(PG_FUNCTION_ARGS)
{
